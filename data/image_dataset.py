    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                ...
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i] or 'RCTW' in self.data_list[i] or 'SynthText' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/test_gts/'+timg.strip()+'.txt' for timg in image_list]
                elif 'MTWI' in self.data_list[i] or 'USTB1K' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/test_gts/'+timg.strip()+'.txt' for timg in image_list]
                else:
    ...
    def load_ann(self):
        ...
        for gt in self.gt_paths:
            ...
            for line in reader:
                ...
                if 'icdar' in self.data_dir[0] or "RCTW" in self.data_dir[0]:
